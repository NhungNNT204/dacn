...
public interface MessageRepository extends JpaRepository<Message, Long> {
    ...
    @Query("SELECT m FROM Message m WHERE m.chatGroupId = :chatGroupId AND m.isDeleted = false " +
    ...
    // Tìm tin nhắn đã pin
    @Query("SELECT m FROM Message m WHERE m.chatGroupId = :chatGroupId AND m.isPinned = true " +
    ...
    @Query("SELECT m FROM Message m WHERE m.chatGroupId = :chatGroupId AND " +
    ...
    @Query("SELECT m FROM Message m WHERE m.chatGroupId = :chatGroupId AND m.senderId = :senderId " +
           "AND m.isDeleted = false ORDER BY m.createdAt DESC")
    List<Message> findByUserInGroup(@Param("chatGroupId") Long chatGroupId, @Param("senderId") Long senderId);
    
    // Tính số tin nhắn chưa đọc
    @Query("SELECT COUNT(m) FROM Message m WHERE m.chatGroupId = :chatGroupId AND m.createdAt > :lastReadAt")
